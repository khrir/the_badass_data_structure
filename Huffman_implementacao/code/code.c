...
void escrv_bytes_cod(char *nome_arquivo, Node_prio *fila){
    ...
    FILE *arq_cod = fopen("tmp/bytes_code.txt", "wb");
    uchar byte_lido;
    size_t result;
...
void salvar_no_arquivo(char *nome_arquivo, int tamanho_arvore, Fila_prio *fila){
    ...
    FILE *arq_codificado = fopen("tmp/bytes_code.txt", "rb");
    int tmn_lixo = calc_lixo("tmp/bytes_code.txt");
    puts("Lixo calculado!");
    ...
    FILE *arq_cod = fopen("tmp/caminho_bytes.txt", "rb");
    uchar byte;
    size_t result;
